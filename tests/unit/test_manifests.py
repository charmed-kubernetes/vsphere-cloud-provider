...
def test_hashable_resource(namespace):
    ...
    hr = _HashableResource(None, rsc_obj)
    assert str(hr) == f"MagicMock/{namespace+'/' if namespace else ''}test-resource"

    hr2 = _HashableResource(None, rsc_obj)
    assert hr == hr2
    assert len({hr, hr2}) == 1
...
def test_resources_version(test_manifest, release, uniqs):
    ...
    assert element.metadata.name == "test-manifest-manager"
...
def test_delete_one_resource(test_manifest, caplog):
    ...
    mock_client.delete.assert_called_once_with(
        type(element.rsc), "test-manifest-secret", namespace="kube-system"
    )
